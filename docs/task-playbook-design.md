...
> **建议**：**先 A 后 B**。P1 用方案 A 把"手册 + 编辑工具 + Exa 参数 + 编辑时翻译成抓取配置"跑通，拿到 90% 价值且零运行时风险；待确有"动态热点"类需求，再上方案 B 的运行时 agentic 抓取。本 RFC 其余章节以 A 为主线，B 的接口预留在 §8。
>
> Boss 的原话"在 agent 调工具获取信息前读取"更接近 B；但 A 是同一意图的低风险起步（读取发生在编辑期而非运行期）。**这一条请 Boss 明确选 A / B / 先A后B。**
...
新增两个 agent 工具（M4 标准：读工具直接执行、写工具走确认卡）：

- `view_task_playbook(schedule_id)`：读工具，返回手册当前内容 + 翻译出的抓取计划摘要。
- `edit_task_playbook(schedule_id, content)`：写工具（Mutating，确认卡）。Execute 里：
  1. 存 `content`；
  2. **（方案 A）** 立刻由 agent/一次 LLM 调用把 `content` 翻译成 `fetch_plan`（结构化），经 sourcespec 校验后落库；翻译失败回自纠文案不落库。
  3. 归属校验：schedule 必须属于当前 owner（单 owner MVP 下天然成立，仍按 user 维度校验）。

> 自然语言编辑闭环：用户在飞书说"这个早报以后只要 OpenAI/Anthropic 官方博客"，agent 调 `edit_task_playbook` → 出确认卡 → 用户确认 → 手册与抓取计划更新。下次该任务跑就按新计划抓。
...
| 阶段 | 内容 | 价值 / 风险 |
|---|---|---|
| **P0** | 迁移建表 + `view/edit_task_playbook` 工具 + 手册存取（暂不影响抓取） | 能存能改能看，零运行时风险 |
| **P1（方案 A）** | 编辑时翻译成 `fetch_plan` + Fetch 按计划抓 + Exa 参数入 sourcespec（修 D-2）+ 改 M6 §5.3（定 D-5） | 拿到主要价值：手册真正决定抓什么；确定性、低成本 |
...
1. **§3 A/B**：手册"读取"发生在编辑期（A，推荐先做）还是每次运行期（B）？还是先 A 后 B？
2. **§4 作用层**：本期只作用于抓取选材（推荐），还是也要影响打分/出卡（碰 M5）？
3. **手册 vs 订阅源**：有手册的任务，抓取是**只按手册计划**（忽略其它订阅源）还是**手册优先 + 订阅源兜底**？
4. **无手册的任务**：保持现状（抓全部订阅源）——确认此向后兼容行为可接受。
5. **D-5**：确认改 M6 §5.3 认可 lookback 显式支持（手册教慎用），而非彻底禁。
6. **成本护栏**：若走 B，每个定时任务每次多一次多轮 LLM——是否要限额（连回审计 R-1 的 token 预算话题）。
