...
- owner 普通聊天只装配 `query_my_intelligence`、`manage_tasks`、经统一裁决的 `update_profile`、`recall_memory`、`manage_memory` 与已配置的公开研究工具。长期记忆只收当前 owner 显式记住/纠正/忘记，BM25 召回结果不构成指令或写权限。构造时缺少 exact evidence writer、任一必需工具或混入八个旧任务工具都会拒绝启动。
