...
claim conflicts. P1-B will own lifecycle wiring and termination recovery.
...
- title and the existing channel-neutral Markdown body (the Store compares the
  body and discovery time byte-for-byte with the durable delivery);
...
`vane_app`, and settable only by the migration owner. It can:

- read only the run/batch/delivery identity columns needed for admission;
