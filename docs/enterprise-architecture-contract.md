...
| D2 | **认证 = Casdoor 式商用注册**，支持国内外社交（X / Facebook / Google / 微信 / QQ 等） | 引入独立 IdP，principal 来源改为 OIDC；vane 不自建密码体系 |
...
**未拍板、按默认执行（可随时推翻）**:
- Casdoor **自托管**（同 VPS docker-compose，Go 单体，与现有栈一致），非 Casdoor Cloud。
...
| 接缝 | 内容 | 依赖 |
|---|---|---|
| **① 认证与租户**（D1/D2/D4/D8/D9） | Casdoor 接入、tenants/memberships、邀请码、注销 | **无（地基）** |
...
### §1.1 认证:Casdoor 作为唯一 principal 来源
...
**契约**:
1. Casdoor 自托管，负责注册/登录/社交 IdP 联邦（X / Facebook / Google / 微信 / QQ）/
   密码重置 / MFA。**vane 不自建密码体系**（不存密码哈希、不做重置流）。
2. vane 侧只做 OIDC RP:校验 Casdoor 签发的 token → 得到 `sub`（Casdoor 用户 ID）→
   映射到本地 `users` 行。
3. principal 解析收敛为**唯一一处**:
   ```go
...
**注册流**:Casdoor 认证成功 → vane 校验邀请码（未消费/未过期/未超次）→
...
1. **首选 magic link**:一次性 token + 15–30 分钟过期 + 高熵不可猜 + 执行前二次认证
   （邮箱可能被转发/代收，token ≠ 身份；D2 下二次认证直接走 Casdoor）。
...
第 0 步（串行，零风险）  §1.1 principal 收敛（过渡期返回固定值，行为不变）
        ↓
第 1 步（串行，地基）    接缝①：Casdoor 接入 + tenants/memberships/invites + 注册流
        ↓
第 2 步（并行扇出）
...
| **vane 自建密码体系** | D2 已定 Casdoor：密码哈希/重置/MFA/社交联邦是 IdP 的职责，自建等于重造轮子并自负安全责任 |
| **飞书应用市场应用** | D5 已定自建应用：市场应用要过审（周期不可控）、所有租户共享 API 限流（跨租户互伤）、Manager 要做多租户事件分流（改造量最大） |
