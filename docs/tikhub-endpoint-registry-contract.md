...
- **机制**：2xx 响应超单次内联上限（下方派生值）时，全量进进程内 LRU 缓存（句柄 `res-N`、
...
- **内联上限由模型的上下文窗口派生，不再是常量（2026-07-18，OpenClaw 同款）**：
  ...
  | ≥100k | 32,000 | 96,000 |
  | ≥200k | 64,000 | 192,000 |
  | ≥1M（当前 deepseek-v4-pro） | **100,000** | 300,000 |

  封顶规则：`PerCall ≤ 窗口 × 30% × 2 字符/token`（30% 同 OpenClaw；字符-token 比
  取 2 而非其 4，因为我们的工具结果是中文社媒 JSON，取小是保守的一侧）。
  未登记模型取**保守兜底窗口 64k**——猜小只让上限偏小（内容仍可句柄取回），
  猜大则直接打爆请求。

  **为什么必须派生而不是写死**：原值 6000 rune 是 64K 窗口时代拍的，模型换到 1M
  后无人记得改，直到 Boss 在生产撞见截断才发现差了一个数量级（业界横比：Claude Code
  MCP 25k tokens / Pi 50KB / OpenClaw 16–64k chars / Codex 10k tokens，我们窗口最大、
  上限最小）。窗口是模型属性，上限就该跟着模型走。
- **每消息累计预算的存在理由**：单次上限管不住 msgCap=10 次调用的总量，更管不住
  FC 多轮把历史重发 M 遍的成本乘数（MaxTurns=20，前缀缓存打折但不改数量级）。
  故按曲线分配：前几次给满、耗尽后降到保底（仍带句柄，不是数据丢失，只是首屏变窄）。
  续读 offset 用**本次实际展示量**而非上限常量——降级后用常量会让模型跳过中间一段。
  A2A/RunOnce 无 per-message 状态，按单次满额、不累计。
- `read_endpoint_result`（静态白名单成员，读工具，**本地缓存读取不打上游、
  ...
  优先于 offset 分页；单次返回封顶同为派生的 `PerCall`，超限给下一步 offset。
...
- 护栏：单次 20s 超时；响应体读取上限 2 MiB（内存护栏，超限 Result.Truncated=true
  由调用方诚实告知）；回模型另有按模型窗口派生的内联上限（token 护栏，§3.5），
  两层各管各的。固定可信主机 api.tikhub.io，无 SSRF 面。
