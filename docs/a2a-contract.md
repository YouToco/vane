...
> **工具面修订（2026-07-29）：** A2A `assistant.chat` 只保留任务级只读能力；
> 本文中的 `list_sources` 说明已退役，不能作为重新暴露内部抓取目标的依据。
>
...
按 AGENTS.md「核心路径/跨包契约变更上全流程」，PR-4（assistant.chat）经两怀疑者对抗审查（视角：A=并发/生命周期，B=安全/协议）。真发现全部已修，逐条：
