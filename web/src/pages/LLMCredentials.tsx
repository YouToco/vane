    ...
    <Card>
      <CardHeader className="space-y-2">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <CardTitle className="flex items-center gap-2 text-base">
            <KeyRound className="size-4" />共享 LLM 凭证
          </CardTitle>
        ...
        <p className="text-sm text-muted-foreground">
          仅超级管理员可修改。API Key 以 AES-GCM 密文进入数据库，页面不会读取或回显原值。
        </p>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-2"><Label htmlFor="llm-provider">Provider</Label><Input id="llm-provider" value="deepseek" disabled /></div>
          <div className="space-y-2"><Label htmlFor="llm-base-url">兼容 API 地址</Label><Input id="llm-base-url" value={form.base_url} onChange={(e) => update("base_url", e.target.value)} /></div>
          <div className="space-y-2 md:col-span-2"><Label htmlFor="llm-api-key">新 API Key</Label><Input id="llm-api-key" type="password" autoComplete="new-password" value={form.api_key} onChange={(e) => update("api_key", e.target.value)} placeholder="每次轮换必须重新输入；不会回显旧值" /></div>
        ...
        <div className="flex flex-wrap gap-2">
          <Button onClick={() => void save()} disabled={busy !== "" || !form.api_key || status?.vault_ready === false}>
