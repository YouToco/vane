...
func TestParseAggMetadataPreservesOnlyOneConsistentEffectMarker(t *testing.T) {
	...
	if title != "title" || template != "blue" || gotEffectID != "" {
		t.Fatalf("drifted metadata = %q/%q/%q, want marker rejected", title, template, gotEffectID)
	}
}
