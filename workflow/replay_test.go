...
//
//   - HappyPath：008 之前的历史（跑到 Push 成功）用**当前**代码重放，必须 PASS。
//     这是真正有 in-flight 风险的那条路——Score/CardGen 是 LLM 调用，秒级到十秒级，
//     发布正好撞上时 worker 就得拿新代码重放旧历史。
//   - GateHistoryIsIncompatible：008 之前停在闸门的历史用当前代码重放，必须 FAIL。
...
// 用例都刻意不引用任何 008 新增符号，正是为了能在基线上原样编译通过、供此校准。
//
// **已实跑的校准结果**（2026-07-16，非推演——两个方向都验过）：
//
//	                        基线代码(stash PR2)   当前代码(含 008)
//	HappyPath                    PASS                PASS      ← 不变量：这条路没被动过
//	GateHistoryIsIncompatible    FAIL                PASS      ← 变量：008 的 delta 在此现形
//
// 左列证明夹具忠实于基线（HappyPath 在基线上就能过，说明活动名/顺序/事件 ID/marker
// 全对）；右列与左列**不同**证明这对用例真在测 008 的差异，而不是一对恒真断言。
...
//
// 基线复现方法：`git stash push` 掉 PR2 改动，把引用了 008 新符号的
...
// baselineHappyPathHistory 是一份 **008 之前**的历史：EvolveProfile → Fetch(20 条)
// → Dedup(18) → Score(18) → Select(5) → CardGen(5) → Push → Completed。
// 它里面**没有** RecordEmptyBatch —— 008 之前那个 Activity 还不存在。
...
// TestPushPipelineWorkflow_ReplayBaselineHappyPath 008 **之前**的成功推送历史，
