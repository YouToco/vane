...
func TestAggregateCard_DurableEffectMarkerIsInEveryCallback(t *testing.T) {
	...
	if got := strings.Count(card, `"effect_id":"`+effectID+`"`); got != callbacks {
		t.Fatalf(
			"effect marker count = %d, want every one of %d callbacks: %s",
			got,
			callbacks,
			card,
		)
	...
	legacy := BuildAggregateCard(feedback.AggregateCardInput{
		Items: []feedback.CardInput{{DeliveryID: 7, Title: "x"}},
	})
