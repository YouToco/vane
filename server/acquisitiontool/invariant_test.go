...
// 忘了进幂等键就会在这里红——而不是等到线上有人发现自己的抓取目标不对劲。
func TestInvariant_FetchAffectingParamsChangeIdempotencyKey(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			base:    map[string]string{"page_id": "6301c499df9bea0001dc6f47"},
			variant: map[string]string{"page_id": "5e397e34654fda00010b7321"},
		},
		{
...
func TestCategoriesMalformedIsRejected(t *testing.T) {
	_, errMsg := BuildTarget(Requirement{
...
func mustBuild(t *testing.T, platform, capability string, params map[string]string) *struct {
	...
	t.Helper()
	src, errMsg := BuildTarget(Requirement{Platform: platform, Capability: capability, Params: params})
	if errMsg != "" {
		t.Fatalf("BuildTarget(%s/%s, %v) 失败: %s", platform, capability, params, errMsg)
	}
	return &struct {
		URL    string
		Config []byte
