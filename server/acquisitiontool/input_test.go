...
func TestBuild_WebFeed(t *testing.T) {
	src, msg := BuildTarget(Requirement{
		Platform:   "web",
		Capability: "feed",
...
func TestBuild_WebFeedCategoriesInKey(t *testing.T) {
	...
	a, _ := BuildTarget(Requirement{Platform: "web", Capability: "feed", Params: map[string]string{"url": "https://openai.com/rss.xml"}})
	b, _ := BuildTarget(Requirement{Platform: "web", Capability: "feed", Params: map[string]string{"url": "https://openai.com/rss.xml", "categories": string(catJSON)}})
	if a.URL == b.URL {
...
func TestBuild_WebSearchCategoryInIdempotencyKey(t *testing.T) {
	a, msg := BuildTarget(Requirement{Platform: "web", Capability: "search", Params: map[string]string{"query": "AI", "category": "news"}})
	if msg != "" {
		t.Fatalf("意外报错: %s", msg)
	}
	b, msg := BuildTarget(Requirement{Platform: "web", Capability: "search", Params: map[string]string{"query": "AI", "category": "research paper"}})
	if msg != "" {
		t.Fatalf("意外报错: %s", msg)
	}
	c, msg := BuildTarget(Requirement{Platform: "web", Capability: "search", Params: map[string]string{"query": "AI"}})
	if msg != "" {
		t.Fatalf("意外报错: %s", msg)
	}
	...
	a2, _ := BuildTarget(Requirement{Platform: "web", Capability: "search", Params: map[string]string{"query": "AI", "category": "news"}})
	if a.URL != a2.URL {
...
func TestBuild_WebSearchIncludeDomains(t *testing.T) {
	mk := func(domainsJSON string) *types.FetchTarget {
		p := map[string]string{"query": "Claude release"}
		if domainsJSON != "" {
			p["include_domains"] = domainsJSON
		}
		src, msg := BuildTarget(Requirement{Platform: "web", Capability: "search", Params: p})
		if msg != "" {
			t.Fatalf("意外报错: %s", msg)
		}
		return src
	...
	full, msg := BuildTarget(Requirement{Platform: "web", Capability: "search", Params: map[string]string{
...
func TestBuild_WebSearchIncludeDomainsRejectsBadJSON(t *testing.T) {
	if _, msg := BuildTarget(Requirement{Platform: "web", Capability: "search", Params: map[string]string{
...
func TestBuild_TrimsWhitespace(t *testing.T) {
	a, _ := BuildTarget(Requirement{Platform: "web", Capability: "search", Params: map[string]string{"query": "AI"}})
	b, _ := BuildTarget(Requirement{Platform: "web", Capability: "search", Params: map[string]string{"query": "  AI  "}})
	if a.URL != b.URL {
		t.Errorf("首尾空白应被归一化: %q vs %q", a.URL, b.URL)
	}
	if _, msg := BuildTarget(Requirement{Platform: "web", Capability: "search", Params: map[string]string{"query": "   "}}); msg == "" {
		t.Error("全空白 query 应被拒绝")
	}
	if _, msg := BuildTarget(Requirement{Platform: "xhs", Capability: "search", Params: map[string]string{"keyword": "\t \n"}}); msg == "" {
...
func TestBuild_RejectsOverlongParams(t *testing.T) {
	...
	if _, msg := BuildTarget(Requirement{Platform: "web", Capability: "search", Params: map[string]string{"query": long}}); msg == "" {
		t.Error("超长 query 应被拒绝")
	}
	if _, msg := BuildTarget(Requirement{Platform: "xhs", Capability: "search", Params: map[string]string{"keyword": long}}); msg == "" {
		t.Error("超长 keyword 应被拒绝")
	}
}

func TestBuild_XHSSearch(t *testing.T) {
	src, msg := BuildTarget(Requirement{Platform: "xhs", Capability: "search", Params: map[string]string{"keyword": "AI 创业"}})
	if msg != "" {
		t.Fatalf("意外报错: %s", msg)
	}
	...
	if _, msg := BuildTarget(Requirement{Platform: "xhs", Capability: "search", Params: map[string]string{}}); msg == "" {
		t.Error("缺 keyword 应被拒绝")
	}
}

func TestBuild_XUserPosts(t *testing.T) {
	src, msg := BuildTarget(Requirement{Platform: "x", Capability: "user_posts", Params: map[string]string{"screen_name": "@elonmusk"}})
	if msg != "" {
		t.Fatalf("意外报错: %s", msg)
	}
	...
	if _, msg := BuildTarget(Requirement{Platform: "x", Capability: "user_posts", Params: map[string]string{}}); msg == "" {
...
func TestBuild_XHSUserPosts(t *testing.T) {
	...
	src, msg := BuildTarget(Requirement{Platform: "xhs", Capability: "user_posts", Params: map[string]string{"user_id": uid}})
	if msg != "" {
		t.Fatalf("意外报错: %s", msg)
	}
	...
	src2, msg := BuildTarget(Requirement{Platform: "xhs", Capability: "user_posts", Params: map[string]string{
	...
	// 两者皆缺 → 拒绝。
	if _, msg := BuildTarget(Requirement{Platform: "xhs", Capability: "user_posts", Params: map[string]string{}}); msg == "" {
...
func TestBuild_XSearchUnavailable(t *testing.T) {
	_, msg := BuildTarget(Requirement{Platform: "x", Capability: "search", Params: map[string]string{"query": "AI"}})
	if msg == "" {
...
func TestBuild_UnknownPlatform(t *testing.T) {
	if _, msg := BuildTarget(Requirement{Platform: "carrier_pigeon", Capability: "user_posts"}); msg == "" {
		t.Error("未知 platform 应被拒绝")
	}
}

func TestBuild_WebContents(t *testing.T) {
	src, msg := BuildTarget(Requirement{Platform: "web", Capability: "contents",
	...
	// 缺 url 应报错。
	if _, msg := BuildTarget(Requirement{Platform: "web", Capability: "contents", Params: map[string]string{}}); msg == "" {
		t.Error("web/contents 缺 url 应被拒绝")
	}
	// 非法 url 应报错。
	if _, msg := BuildTarget(Requirement{Platform: "web", Capability: "contents", Params: map[string]string{"url": "not-a-url"}}); msg == "" {
		t.Error("web/contents 非法 url 应被拒绝")
	}
}

// page_watch 已下线（改由 Exa fetch 覆盖）：web 平台不再接受该能力，BuildTarget 应拒绝。
...
func TestBuild_XHSBindingCapabilities(t *testing.T) {
	cases := []struct {
		...
		{"hot_list 无参数", Requirement{Platform: "xhs", Capability: "hot_list"},
			"vane://xhs/hot_list", `{}`},
		{"topic_feed 直填 page_id", Requirement{Platform: "xhs", Capability: "topic_feed",
		...
		{"topic_feed 从深链抽取", Requirement{Platform: "xhs", Capability: "topic_feed",
			...
			"vane://xhs/topic_feed?page_id=6301c499df9bea0001dc6f47",
			`{"page_id":"6301c499df9bea0001dc6f47"}`},
		{"faved_notes 直填 user_id", Requirement{Platform: "xhs", Capability: "faved_notes",
		...
		{"faved_notes 主页链接抽取", Requirement{Platform: "xhs", Capability: "faved_notes",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			src, msg := BuildTarget(tc.spec)
			if msg != "" {
				t.Fatalf("BuildTarget 拒绝: %s", msg)
			}
...
func TestBuild_XHSBindingRejections(t *testing.T) {
	if _, msg := BuildTarget(Requirement{Platform: "xhs", Capability: "topic_feed"}); msg == "" {
		t.Error("topic_feed 缺 page_id 应拒绝")
	}
	if _, msg := BuildTarget(Requirement{Platform: "xhs", Capability: "faved_notes"}); msg == "" {
		t.Error("faved_notes 缺 user_id 应拒绝")
	}
	// 非 hex 输入抽不出 page_id。
	if _, msg := BuildTarget(Requirement{Platform: "xhs", Capability: "topic_feed",
...
func TestBuild_BindingKeyNamespacesDisjoint(t *testing.T) {
	a, _ := BuildTarget(Requirement{Platform: "xhs", Capability: "user_posts",
		Params: map[string]string{"user_id": "69bfda630000000034019ee8"}})
	b, _ := BuildTarget(Requirement{Platform: "xhs", Capability: "faved_notes",
		Params: map[string]string{"user_id": "69bfda630000000034019ee8"}})
	if a == nil || b == nil {
		t.Fatal("BuildTarget 失败")
	}
	if a.URL == b.URL {
...
func TestBuild_WeiboUserPosts(t *testing.T) {
	...
	src, msg := BuildTarget(Requirement{Platform: "weibo", Capability: "user_posts",
	...
	for name, params := range map[string]map[string]string{
		...
		got, msg := BuildTarget(Requirement{Platform: "weibo", Capability: "user_posts", Params: params})
		if msg != "" {
	...
	for name, params := range map[string]map[string]string{
		...
		if _, msg := BuildTarget(Requirement{Platform: "weibo", Capability: "user_posts", Params: params}); msg == "" {
...
func TestBuild_WeiboHotList(t *testing.T) {
	src, msg := BuildTarget(Requirement{Platform: "weibo", Capability: "hot_list"})
	if msg != "" {
		t.Fatalf("意外报错: %s", msg)
	}
...
func TestBuild_WechatMPUserPosts(t *testing.T) {
	...
	src, msg := BuildTarget(Requirement{Platform: "wechat_mp", Capability: "user_posts",
	...
	for name, params := range map[string]map[string]string{
		...
		_, msg := BuildTarget(Requirement{Platform: "wechat_mp", Capability: "user_posts", Params: params})
		if msg == "" {
			t.Errorf("%s 应被拒绝", name)
	...
	// 不支持的能力。
	if _, msg := BuildTarget(Requirement{Platform: "wechat_mp", Capability: "hot_list"}); msg == "" {
