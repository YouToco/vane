...
func TestChannelCredentialFilesFailClosedOnReadErrors(t *testing.T) {
	for _, credentialName := range []string{
		...
		t.Run(credentialName, func(t *testing.T) {
			...
			t.Chdir(t.TempDir())
			directory := t.TempDir()
			// ReadFile on a directory is a deterministic credential read failure.
			if err := os.Mkdir(filepath.Join(directory, credentialName), 0o700); err != nil {
				t.Fatal(err)
			}
			t.Setenv("CREDENTIALS_DIRECTORY", directory)
			t.Setenv("VANE_DB_URL", "postgres://test")
			if _, err := Load(""); err == nil {
				t.Fatal("credential read failure was ignored")
			}
		})
	}
}

func TestCredentialVaultRejectsMalformedIdentifiersAndRetiredKeys(t *testing.T) {
	valid := strings.Repeat("42", 32)
	for _, vault := range []CredentialVaultConfig{
		{ActiveKeyID: "vault.key", ActiveKeyHex: valid},
		{ActiveKeyID: "vault-key", ActiveKeyHex: valid,
			RetiredKeys: "missing-separator"},
		{ActiveKeyID: "vault-key", ActiveKeyHex: valid,
			RetiredKeys: "old-key=" + strings.Repeat("gg", 32)},
	} {
		cfg := Config{DB: DBConfig{URL: "postgres://test"}, CredentialVault: vault}
		if err := cfg.Validate(); err == nil {
			t.Fatalf("unsafe vault config accepted: %+v", vault)
		}
	}
}
