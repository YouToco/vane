...
// Keep the history binding first: production DeepSeek completions otherwise
// tend to copy prose and the nested citations from an older Brief before they
// reach the later citation rules. The outer history record_id is the only
// authority that can support a comparison in the new candidate.
const researchSynthesisHistoryCitationFirstV34 = "先确定 citations，再写 headline 和 summary。只要 definition.output.instructions 要求与历史基线比较，或者输出中出现“与此前相同、发生变化、最近一次、历史基线、持续、仍然、无实质变化”等比较性表述，citations 就必须同时包含直接支持比较的外层 history 记录；ref 必须逐字复制对应 history.items[].record_id。例如 history.items[0].record_id 为 \"146\" 时，使用 {\"kind\":\"history\",\"ref\":\"146\"}。绝不能复制或沿用 history.items[].payload_text 内嵌的旧 citations；它们只属于旧运行。如果不引用直接支持比较的外层 history 记录，就必须删除所有比较性表述和从历史取得的日期，只报告 current_evidence 支持的当前状态。此检查优先于生成任何正文。"
