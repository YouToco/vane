...
const (
	...
	// CapProductStatus reads a supported official pricing/catalog endpoint and
	// emits a stable, structured purchase-state snapshot. It is deliberately
	// separate from CapContents: dynamic product pages may have no crawlable
	// HTML, while their first-party catalog API is still the authoritative fact.
	CapProductStatus Capability = "product_status"
