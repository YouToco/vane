...
func TestCORS_预检不进会话中间件(t *testing.T) {
	...
	if got := h.Get("Access-Control-Allow-Headers"); got != "Content-Type, Idempotency-Key, X-Vane-Reauth-Token" {
		t.Errorf(
			"Allow-Headers = %q, want exact content, idempotency, and reauth headers",
			got,
		)
	}
}
