...
func TestMigration143And144Boundaries(t *testing.T) {
	tests := []struct {
		path      string
		required  []string
		forbidden []string
		...
		{
			path: "migrations/143_agent_conversation_scopes.sql",
		...
		{
			path: "migrations/144_channel_rate_limit_retry.sql",
...
func TestOptionalSchemaCapabilitiesAcrossRetainedMigrationPostgres(t *testing.T) {
	...
	t.Cleanup(drop)
	if _, err := provider.UpTo(t.Context(), 142); err != nil {
		t.Fatal(err)
	}
	...
	if legacy.agentConversationScopes || legacy.channelSendRetry {
		legacy.Close()
		t.Fatalf("migration 142 capabilities=(%t,%t), want false,false",
			legacy.agentConversationScopes, legacy.channelSendRetry)
	}
	userID, tenantID := migration129Identity(t, database, "retained-schema-capabilities")
	if _, err := legacy.GetActiveAgentSession(
		t.Context(), tenantID, userID, time.Now().Add(-time.Hour),
	); !errors.Is(err, types.ErrNotFound) {
		legacy.Close()
		t.Fatalf("legacy empty lookup err=%v", err)
	}
	session, err := legacy.CreateAgentSession(t.Context(), tenantID, userID)
	...
	if _, err := provider.UpTo(t.Context(), 144); err != nil {
		t.Fatal(err)
	}
	...
	if !current.agentConversationScopes || !current.channelSendRetry {
		t.Fatalf("migration 144 capabilities=(%t,%t), want true,true",
...
func TestScopedAgentSessionsPostgres(t *testing.T) {
	...
	userID, tenantID := migration129Identity(t, database, "telegram-session-scope")

	owner, err := st.CreateAgentSession(t.Context(), tenantID, userID)
	if err != nil || owner.ConversationScope != "owner" {
		t.Fatalf("owner=%+v err=%v", owner, err)
	}
	private, err := st.CreateAgentSessionInScope(
		t.Context(), tenantID, userID, "channel-route:11")
	if err != nil {
		t.Fatal(err)
	}
	group, err := st.CreateAgentSessionInScope(
		t.Context(), tenantID, userID, "channel-route:22")
	if err != nil {
		t.Fatal(err)
	}
	for scope, wantID := range map[string]int64{
		...
		got, getErr := st.GetActiveAgentSessionInScope(
			t.Context(), tenantID, userID, scope, time.Now().Add(-time.Hour))
	...
	if _, err := st.CreateAgentSessionInScope(
		t.Context(), tenantID, userID, "telegram chat 42"); !errors.Is(err, types.ErrValidation) {
	...
	if gotOwner, err = st.GetActiveAgentSession(
		t.Context(), tenantID, userID, time.Now().Add(-time.Hour)); err != nil ||
		gotOwner.ID != owner.ID {
		t.Fatalf("scope expiry changed owner=%+v err=%v", gotOwner, err)
	}
	if _, err := provider.DownTo(t.Context(), 142); err == nil ||
