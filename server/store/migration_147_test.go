...
func TestMigration147ChannelMediaEnvelopeBoundary(t *testing.T) {
	payload, err := migrationsFS.ReadFile("migrations/147_channel_media_envelope.sql")
	if err != nil {
		t.Fatal(err)
	}
	text := string(payload)
	for _, required := range []string{
		...
		if !strings.Contains(text, required) {
			t.Errorf("migration 147 lost boundary %q", required)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(text, forbidden) {
			t.Errorf("migration 147 persists forbidden authority %q", forbidden)
		}
	}
}
...
func TestTelegramMediaEnvelopeReplayPostgres(t *testing.T) {
	...
	if _, err := provider.DownTo(t.Context(), 146); err == nil ||
