...
func TestMigration133ChannelBoundary(t *testing.T) {
	payload, err := migrationsFS.ReadFile("migrations/133_channel_ingress.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, fragment := range []string{
		...
		if !strings.Contains(sqlText, fragment) {
			t.Errorf("migration 133 lost boundary %q", fragment)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(sqlText, forbidden) {
			t.Errorf("migration 133 introduced unsafe coupling %q", forbidden)
		}
	}
}
