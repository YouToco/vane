...
func TestMigration142ChannelRouteBoundary(t *testing.T) {
	payload, err := migrationsFS.ReadFile("migrations/142_channel_routes.sql")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, fragment := range []string{
		...
		if !strings.Contains(sqlText, fragment) {
			t.Errorf("migration 142 lost boundary %q", fragment)
		}
	}
	for _, forbidden := range []string{
		...
		if strings.Contains(sqlText, forbidden) {
			t.Errorf("migration 142 introduced unsafe coupling %q", forbidden)
		}
	}
}
