...
const latestMigrationVersion int64 = 155
...
var wantTables = []string{
	...
	"artifact_delivery_plans",
	"aggregate_channel_delivery_effects",
	// 015 agent 工具调用记账（TikHub 端点注册表契约 §6）
	"tool_calls",
	// 082 immutable audit ledger for sensitive platform-owner trace reads.
	"admin_trace_access_events",
	// 080 provider/model/endpoint price history and ledger calculation authority.
	...
	// 086 immutable V3 per-run research plan and Tool step receipts.
	"research_run_plans",
	"research_run_steps",
	"research_run_evidence",
	// 090 immutable first-writer spend authority and append-only settlement.
	...
	"research_llm_gateway_verifier_keys",
	"research_llm_gateway_attempts",
	"research_llm_gateway_frozen_requests",
	...
	// 126 immutable planner tool-search receipts and loaded-tool admission.
	...
	// 129 explicit owner-authorized long-term memory ledger.
	"memory_records",
	"memory_events",
	"memory_receipts",
	...
	// 155 fail-closed, FORCE-RLS admission proof for stored channel runtimes.
