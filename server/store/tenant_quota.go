...
//
// 取值依据：
//   - llm_tokens 在 2026-08-08 的 Agent-first 真实任务验收中证明会因保守的
//     最大输出预留而产生假性耗尽。Boss 决定在当前开发阶段把默认值提高到
//     10 亿 token/天，使它不再阻断正常任务和影子验收；供应商账户余额、
//     单次调用上限、不可变成本账本和幂等闸门仍独立生效。
...
var defaultQuotas = []QuotaDefault{
	{QuotaLLMTokens, 1_000_000_000.0 / 86400, 1_000_000_000},
