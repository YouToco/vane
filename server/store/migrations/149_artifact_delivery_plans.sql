-- 149: freeze one business artifact's provider fan-out before any send.
--
...
CREATE FUNCTION lock_artifact_delivery_telegram_route_v1(
    p_tenant_id BIGINT,
    p_user_id BIGINT,
    p_route_id BIGINT
...
AS $$
    SELECT cr.id
     ...
     WHERE p_tenant_id=
       ...
       AND cr.tenant_id=p_tenant_id AND cr.user_id=p_user_id
       AND cr.provider='telegram' AND cr.status='active'
       AND ci.status='active'
       AND ((p_route_id IS NULL AND cr.route_kind='private') OR
            cr.id=p_route_id)
     ORDER BY cr.bound_at,cr.id
     LIMIT 1
       FOR UPDATE OF cr
$$;
-- +goose StatementEnd
REVOKE ALL ON FUNCTION
    lock_artifact_delivery_telegram_route_v1(BIGINT,BIGINT,BIGINT)
    FROM PUBLIC;
...
BEGIN
    IF EXISTS (SELECT 1 FROM artifact_delivery_plans) THEN
        RAISE EXCEPTION '149: refusing downgrade while frozen artifact delivery plans exist';
    END IF;
END $$;
-- +goose StatementEnd
DROP FUNCTION
