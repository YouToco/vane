-- 141: authenticated external-channel identities and durable ingress receipts.
--
...
BEGIN
    IF EXISTS (SELECT 1 FROM channel_ingress_receipts) OR
       ...
       EXISTS (SELECT 1 FROM channel_identities) THEN
        RAISE EXCEPTION '141: refusing downgrade while Telegram channel history exists';
    END IF;
END $$;
-- +goose StatementEnd
