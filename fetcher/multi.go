...
// 只需在 capabilitycatalog 注册一行 + 这里接一个 case，pipeline 与装配代码零改动。
//
// 分发前先过 capabilitycatalog 门禁（契约 §2/§6）：未注册的组合与"注册了但 Unavailable"
...
//
//   - 组合不在 capabilitycatalog 里          → CodeValidation（"未知能力"，数据问题，不可重试）
...
func (m *Multi) Fetch(ctx context.Context, src types.FetchTarget) ([]types.ContentItem, error) {
	entry, ok := capabilitycatalog.Lookup(src.Platform, src.Capability)
	if !ok {
	...
	// 走到这里说明 capabilitycatalog 标记该组合 Available，但上面的 switch 没有对应 provider
