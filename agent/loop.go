...
- create_schedule 必须带完整 intent 与 approved_fetch_plan。抓取要求只用 fetch_requirements（固定 version=vane.fetch-requirements/v1）提交人类可读的原始参数；绝不能引用历史账号对象，也不能编造 config、selectors、vane:// URL 或内部 id。系统会确定性物化并冻结任务自己的长期抓取计划，不能自行扩大主题或替换目标。需要先上网找候选时，本轮只能做只读发现并把候选告诉用户；下一条消息再根据用户明确选择创建，绝不能在读取外部结果的同一轮发起写操作。
...
- 用户已明确要求按本条消息直接创建任务。本轮不注入画像，也不要询问行业、职业、岗位或更新画像。不要调用 list_schedules、web_search、read_page 或其他读取工具；只能使用本条用户消息中明确提供的信息调用 create_schedule，不得用历史或画像。新长期抓取要求直接写入 approved_fetch_plan：version 固定为 vane.fetch-requirements/v1，items 只提交 kind 对应的人类可读参数；绝不能编写 config、selectors、vane:// URL 或任何内部 id。唯一允许的有界补全是：用户明确点名机构且要求官方来源时，可填写这些机构对应的官方裸域名；不得加入未点名机构、媒体或社区。若信息确实不足，不得编造，应自然追问；没有实际调用 create_schedule 就绝不能声称任务已创建。`
...
func projectDirectTaskCreationToolDef(def llm.ToolDef) (llm.ToolDef, bool) {
	...
	fetchRequirements, ok := planProperties["fetch_requirements"]
	if !ok {
		return llm.ToolDef{}, false
	}
	fetchRequirementsObject, ok := fetchRequirements.(map[string]any)
	if !ok {
		return llm.ToolDef{}, false
	}
	approved["properties"] = fetchRequirementsObject["properties"]
	approved["required"] = fetchRequirementsObject["required"]
	approved["additionalProperties"] = false
	approved["description"] = "直接创建模式只接受本条用户消息指定的取材目标：version 固定为 vane.fetch-requirements/v1，items 只含人类可读参数。"
	projected, err := json.Marshal(schema)
	if err != nil {
		return llm.ToolDef{}, false
	}
	def.Description = "按当前用户消息直接创建任务。取材目标直接填入 approved_fetch_plan 的 version/items；系统会冻结任务定义并自动推进。"
...
// 生成的嵌套参数原样保留；只有精确匹配的扁平 plan 才会被包进 fetch_requirements。
...
func normalizeTaskCreationArgs(args json.RawMessage) (json.RawMessage, bool) {
	...
	wrappedPlan, err := json.Marshal(struct {
		FetchRequirements json.RawMessage `json:"fetch_requirements"`
	}{
		FetchRequirements: envelope.ApprovedFetchPlan,
	})
	if err != nil {
		return nil, false
	}
...
func inspectModelTaskCreationPlan(
	...
	var plan struct {
		FetchRequirements json.RawMessage `json:"fetch_requirements,omitempty"`
	}
