...
// beginObservedEventAdmissionV1 is the observation/effect admission boundary.
// Unlike ordinary compiled writes it must join schema and tenant admission
// before reading any run-owned row, then fence the exact effect-authority
// batch before locking the live schedule and event claim.
func (s *Store) beginObservedEventAdmissionV1(
	...
	if err != nil {
		return nil, nil, taskRunDatabaseError(
			"begin observed event admission",
			err,
		)
	}
	if err := lockPushEffectSchemaWriter(ctx, tx); err != nil {
		...
		return nil, nil, taskRunDatabaseError(
			"lock observed event schema admission",
			err,
		)
	}
	if _, err := tx.Exec(ctx,
		...
		rollbackCompiledTaskTx(ctx, tx)
		return nil, nil, taskRunDatabaseError(
			"set observed event tenant context",
			err,
		)
	}
	...
	if err != nil {
		rollbackCompiledTaskTx(ctx, tx)
		return nil, nil, taskRunDatabaseError(
			"lock observed event tenant admission",
			err,
		)
	}
	if !tenantExists {
	...
	if _, err := tx.Exec(ctx, `SET LOCAL ROLE vane_app`); err != nil {
		rollbackCompiledTaskTx(ctx, tx)
		return nil, nil, taskRunDatabaseError(
			"enter observed event application role",
			err,
		)
	}
	...
	for _, scope := range batchScopes {
		...
		status, lockErr := lockPushEffectBatchAdmission(
			ctx, tx, scope, snapshotByBatch[scope.BatchID], requiredAuthority,
		)
		if lockErr != nil {
			rollbackCompiledTaskTx(ctx, tx)
