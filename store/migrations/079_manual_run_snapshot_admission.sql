-- 079: admit a Source-free Tool snapshot for a paused task only when the
-- Temporal workflow identity names an exact live manual run command.
--
-- The application already checks the same tenant/user/task/kind/status tuple
-- while loading the approved definition and authorizing side effects. Keeping
-- the database admission fence aligned prevents a paused recurring cadence
-- from running while still allowing its explicit one-off command.
...
DECLARE
    ...
    exact_manual_run BOOLEAN := FALSE;
BEGIN
    ...
    IF schedule_status = 'paused' THEN
        PERFORM 1
         ...
         WHERE NEW.temporal_workflow_id = 'wf-manual-' || c.id::text
...
-- Manual snapshots may already depend on this admission rule. Restoring the
-- scheduled-only function would create a split-brain authorization contract.
-- +goose StatementBegin
DO $$
BEGIN
    RAISE EXCEPTION
