-- 008_empty_batch_exit.sql — 空批次可见化（M5 契约 §16 修订记录「空批次缺口（归 PR2）」）
--
-- 背景：pipeline 有五处提前退出（见 workflow/workflow.go 里五处 recordEmpty 调用——无新内容 /
-- 去重后空 / 打分后空 / 择优后空 / 卡片生成后空）全部在 Push 之前 `return nil`；而在 008 之前，
...
-- 文档，而它引用的代码会一直漂。008 初稿正是照着改动前的行号写的，PR 自身插入代码后当场失效。
...
-- 退出闸门。DEFAULT '' 的语义是"没有提前退出（跑到了 Push）"——008 之前的历史行
...
-- 实测 down/re-up 循环确认：DROP COLUMN 删的是两个列，而 008 期间写下的
