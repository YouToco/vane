-- 122: independent, evidence-bound grounding verification for V3.3 Briefs.
--
-- The first synthesis completion remains a candidate until a separately
-- frozen no-Tool verifier confirms that every factual claim is entailed by
-- the candidate's actual citations. Verification has its own quota-backed,
-- immutable LLM receipt (synthesis round 1); old V3/V3.1/V3.2 snapshots keep
-- their byte-frozen round-0 replay behavior.
...
CREATE TABLE research_brief_grounding_verifications (
    ...
    candidate_brief_payload           BYTEA NOT NULL,
    candidate_digest                  TEXT NOT NULL,
    verifier_prompt                   BYTEA NOT NULL,
    verifier_prompt_digest            TEXT NOT NULL,
    ...
    CONSTRAINT uq_research_brief_grounding_snapshot UNIQUE(run_snapshot_id),
    CONSTRAINT uq_research_brief_grounding_plan UNIQUE(plan_id),
    CONSTRAINT uq_research_brief_grounding_synthesis UNIQUE(synthesis_id),
    CONSTRAINT uq_research_brief_grounding_reservation
        UNIQUE(verifier_llm_spend_reservation_id),
    CONSTRAINT ck_research_brief_grounding_identity CHECK (
        btrim(task_id)=task_id AND octet_length(task_id) BETWEEN 1 AND 255
    ),
    CONSTRAINT ck_research_brief_grounding_digests CHECK (
        candidate_digest ~ '^[0-9a-f]{64}$' AND
        verifier_prompt_digest ~ '^[0-9a-f]{64}$' AND
        (verdict_digest IS NULL OR verdict_digest ~ '^[0-9a-f]{64}$') AND
        candidate_digest=encode(sha256(candidate_brief_payload),'hex') AND
        verifier_prompt_digest=encode(sha256(verifier_prompt),'hex') AND
        (verdict_payload IS NULL OR verdict_digest=encode(sha256(verdict_payload),'hex'))
    ),
    CONSTRAINT ck_research_brief_grounding_payloads CHECK (
        octet_length(candidate_brief_payload) BETWEEN 2 AND 262144 AND
        octet_length(verifier_prompt) BETWEEN 2 AND 2097152 AND
        position(decode('00','hex') in candidate_brief_payload)=0 AND
        position(decode('00','hex') in verifier_prompt)=0 AND
    ...
    CONSTRAINT ck_research_brief_grounding_status CHECK (
        status IN ('prepared','grounded','rejected')
    ),
    CONSTRAINT ck_research_brief_grounding_shape CHECK (
        (status='prepared' AND verifier_llm_spend_reservation_id IS NULL AND
         verdict_payload IS NULL AND verdict_digest IS NULL AND finalized_at IS NULL) OR
        (status IN ('grounded','rejected') AND
         verifier_llm_spend_reservation_id IS NOT NULL AND
         verdict_payload IS NOT NULL AND verdict_digest IS NOT NULL AND
         finalized_at IS NOT NULL)
    ),
    CONSTRAINT ck_research_brief_grounding_schema CHECK (
        schema_version='vane.research-grounding-verification/v1'
    )
);

CREATE INDEX idx_research_brief_grounding_scope
    ON research_brief_grounding_verifications
       (tenant_id,user_id,task_id,run_snapshot_id,id);

-- +goose StatementBegin
CREATE FUNCTION protect_research_brief_grounding_verification_v1()
RETURNS trigger
LANGUAGE plpgsql
...
BEGIN
    IF ROW(
        ...
        NEW.plan_id,NEW.synthesis_id,NEW.candidate_brief_payload,
        NEW.candidate_digest,NEW.verifier_prompt,NEW.verifier_prompt_digest,
        NEW.schema_version,NEW.created_at
    ) IS DISTINCT FROM ROW(
        OLD.id,OLD.tenant_id,OLD.user_id,OLD.task_id,OLD.run_snapshot_id,
        OLD.plan_id,OLD.synthesis_id,OLD.candidate_brief_payload,
        OLD.candidate_digest,OLD.verifier_prompt,OLD.verifier_prompt_digest,
        OLD.schema_version,OLD.created_at
    ) OR OLD.status<>'prepared' OR NEW.status NOT IN ('grounded','rejected') THEN
        RAISE EXCEPTION '122: research grounding verification is immutable';
    END IF;
...
$$;
-- +goose StatementEnd
REVOKE ALL ON FUNCTION protect_research_brief_grounding_verification_v1() FROM PUBLIC;
CREATE TRIGGER protect_research_brief_grounding_verification_v1
BEFORE UPDATE ON research_brief_grounding_verifications
FOR EACH ROW EXECUTE FUNCTION protect_research_brief_grounding_verification_v1();

GRANT SELECT ON research_brief_grounding_verifications
...
) ON research_brief_grounding_verifications TO vane_app,vane_research_v3_executor;
GRANT UPDATE (
    status,verifier_llm_spend_reservation_id,verdict_payload,verdict_digest
) ON research_brief_grounding_verifications TO vane_app,vane_research_v3_executor;
GRANT USAGE,SELECT ON SEQUENCE research_brief_grounding_verifications_id_seq
    TO vane_app,vane_research_v3_executor;

ALTER TABLE research_brief_grounding_verifications ENABLE ROW LEVEL SECURITY;
CREATE POLICY tenant_visible ON research_brief_grounding_verifications
    FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY tenant_isolation ON research_brief_grounding_verifications AS RESTRICTIVE
    FOR ALL
    ...
    WITH CHECK (tenant_id IS NOT DISTINCT FROM
                NULLIF((SELECT current_setting('app.tenant_id',true)),'')::bigint);
CREATE POLICY user_isolation ON research_brief_grounding_verifications AS RESTRICTIVE
    FOR ALL
    ...
    WITH CHECK (user_id IS NOT DISTINCT FROM
                NULLIF((SELECT current_setting('app.user_id',true)),'')::bigint);
CREATE POLICY research_v3_scope ON research_brief_grounding_verifications AS RESTRICTIVE
...
CREATE POLICY research_v3_capability_scope
    ON research_brief_grounding_verifications AS RESTRICTIVE
    FOR ALL TO vane_research_v3_executor
...
BEGIN
    ...
    IF snapshot_json IS NULL THEN
        RAISE EXCEPTION '122: LLM reservation snapshot scope differs'
            USING ERRCODE='23514';
    END IF;
    stage_key := CASE
    ...
    IF stage_key IS NULL THEN
        RAISE EXCEPTION '122: LLM reservation stage differs'
            USING ERRCODE='23514';
    END IF;
    ...
    IF stage_model IS DISTINCT FROM NEW.model OR stage_max_tokens IS NULL OR
       ...
       snapshot_json #>> '{research_model,quota_bucket}'<>NEW.quota_bucket THEN
        RAISE EXCEPTION '122: LLM reservation differs from frozen model policy'
            USING ERRCODE='23514';
    END IF;
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '122: LLM reservation differs from exact price ceiling'
            USING ERRCODE='23514';
    END IF;
    IF NEW.stage='synthesis' THEN
        IF NOT EXISTS (
            ...
            RAISE EXCEPTION '122: synthesis reservation subject differs'
                USING ERRCODE='23514';
        END IF;
        RETURN NEW;
    END IF;
    ...
    IF NEW.round_ordinal>=max_rounds OR
       ...
       used_cost+NEW.reserved_cost_micro_usd>max_cost THEN
        RAISE EXCEPTION '122: planner reservation exceeds frozen PlannerBudget'
            USING ERRCODE='23514';
    END IF;
    RETURN NEW;
END
...
-- Existing rounds retain the original 091 admission function. Only the new
-- verifier round has a distinct prompt authority and policy key.
-- +goose StatementBegin
CREATE FUNCTION admit_research_run_llm_spend_v4(
...
BEGIN
    ...
    IF requested_tenant_id IS NULL OR requested_user_id IS NULL OR
       ...
       NOT EXISTS (
        ...
        RAISE EXCEPTION '122: verifier LLM admission scope is invalid'
            USING ERRCODE='42501';
    END IF;
    SELECT convert_from(snapshot.payload,'UTF8')::jsonb,
    ...
    IF snapshot_json IS NULL OR
       snapshot_json #>> '{research_model,synthesis,renderer_version}'<>
           'research-synthesis.render/v3.3' OR NOT EXISTS (
        SELECT 1 FROM public.research_brief_grounding_verifications grounding
    ...
    IF stage_model IS NULL OR stage_system_prompt IS NULL OR
       derived_quota_tokens NOT BETWEEN 1 AND 1000000 THEN
        RAISE EXCEPTION '122: verifier frozen policy is invalid'
            USING ERRCODE='23514';
    END IF;
    ...
    IF admitted_id IS NOT NULL THEN
        IF NOT EXISTS (
            ...
            RAISE EXCEPTION '122: verifier LLM admission replay differs'
                USING ERRCODE='23514';
        END IF;
    ...
    IF reserved_cost IS NULL OR reserved_cost<=0 THEN
        RAISE EXCEPTION '122: verifier price ceiling is unavailable'
            USING ERRCODE='23514';
    END IF;
    UPDATE public.tenant_quota quota
     ...
     WHERE quota.tenant_id=requested_tenant_id AND quota.bucket='llm_tokens'
       AND LEAST(quota.burst,
           ...
           >=derived_quota_tokens;
    IF NOT FOUND THEN
        RAISE EXCEPTION '122: verifier LLM quota admission denied'
            USING ERRCODE='P0001';
    END IF;
...
$$;
-- +goose StatementEnd
REVOKE ALL ON FUNCTION admit_research_run_llm_spend_v4(
    BIGINT,BIGINT,TEXT,BIGINT,TEXT,INTEGER,BIGINT,TEXT,TEXT,TEXT,TEXT
) FROM PUBLIC;

-- +goose StatementBegin
CREATE FUNCTION admit_research_run_llm_spend_cap_v4(
    requested_tenant_id BIGINT,
    requested_user_id BIGINT,
    requested_task_id TEXT,
    requested_run_snapshot_id BIGINT,
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION '122: LLM admission capability differs'
            USING ERRCODE='42501';
    END IF;
    RETURN QUERY SELECT * FROM public.admit_research_run_llm_spend_v4(
        requested_tenant_id,requested_user_id,requested_task_id,
        requested_run_snapshot_id,requested_stage,requested_round_ordinal,
        requested_subject_id,requested_attempt_key,requested_request_digest,
        requested_trace_id,requested_user_prompt);
...
GRANT EXECUTE ON FUNCTION admit_research_run_llm_spend_cap_v4(
    BIGINT,BIGINT,TEXT,BIGINT,TEXT,INTEGER,BIGINT,TEXT,TEXT,TEXT,TEXT
...
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM research_brief_grounding_verifications) OR
       EXISTS (
           SELECT 1 FROM research_run_llm_spend_reservations
            WHERE stage='synthesis' AND round_ordinal=1
       ) THEN
        RAISE EXCEPTION '122 down refused: grounding verification history exists';
    END IF;
END
$$;
-- +goose StatementEnd
...
DROP FUNCTION admit_research_run_llm_spend_v4(
    BIGINT,BIGINT,TEXT,BIGINT,TEXT,INTEGER,BIGINT,TEXT,TEXT,TEXT,TEXT
);

DROP TRIGGER protect_research_brief_grounding_verification_v1
    ON research_brief_grounding_verifications;
DROP FUNCTION protect_research_brief_grounding_verification_v1();
DROP TABLE research_brief_grounding_verifications;

-- Restore the exact pre-122 reservation trigger. A down migration must leave
-- the old schema operational, not merely reject round 1 while retaining a
-- function that references the table removed above.
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION enforce_research_run_llm_spend_reservation_v1()
RETURNS trigger
LANGUAGE plpgsql
