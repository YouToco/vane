	...
	tests := []struct {
	...
	}{
		{
			...
			call: func(st *Store) error {
				_, err := st.ListDueSourcesByIDs(t.Context(), nil)
				return err
			},
		},
		{
			...
			call: func(st *Store) error {
				_, err := st.ListDueSourcesByIDs(t.Context(), []int64{1, 0})
				return err
			},
		},
		{
			...
			call: func(st *Store) error {
				_, err := st.ListDueSourcesByIDs(t.Context(), []int64{-1})
				return err
			},
		},
		{
			...
			call: func(st *Store) error {
				_, err := st.ListDueSourcesByIDs(t.Context(), []int64{2, 2})
				return err
			},
		},
		{
		...
		},
		{
			...
			call: func(st *Store) error {
				_, _, err := st.SourceForContentFromIDs(t.Context(), 0, []int64{1})
				return err
			},
		},
		{
			...
			call: func(st *Store) error {
				_, _, err := st.SourceForContentFromIDs(t.Context(), 1, nil)
				return err
			},
		},
		{
			...
			call: func(st *Store) error {
				_, _, err := st.SourceForContentFromIDs(t.Context(), 1, []int64{3, 3})
				return err
			},
		},
	}
...
func TestCompiledRunSourceQueriesDoNotExpandMutableScope(t *testing.T) {
	...
	file, err := parser.ParseFile(fset, "compiled_run_sources.go", nil, 0)
	if err != nil {
		t.Fatalf("parse compiled query source: %v", err)
	}

	forbidden := []string{"schedule_sources", "subscriptions"}
...
func TestCompiledRunSourceQueries(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		if len(sourceIDs) > 0 {
			cleanupExec(cleanupCtx, t, st, `DELETE FROM sources WHERE id = ANY($1)`, sourceIDs)
		}
	...
	createdSources := make(map[int64]types.Source)
	newSource := func(name string) int64 {
		...
		id, _, err := st.UpsertSource(ctx, &source)
		if err != nil {
	...
	planned := make([]compiledPlanSource, 0, 2)
	...
	if _, err := st.pool.Exec(ctx,
		`UPDATE sources
		  ...
		  WHERE id = $1`, sourceA, types.SourceStatusActive, 4); err != nil {
		t.Fatalf("prepare source A health: %v", err)
	}
	if _, err := st.pool.Exec(ctx,
		`UPDATE sources
		    SET status = $2, next_fetch_at = now() - interval '1 minute', fail_count = $3
		  WHERE id = $1`, sourceB, types.SourceStatusActive, 8); err != nil {
		t.Fatalf("prepare source B health: %v", err)
	}
	if _, err := st.pool.Exec(ctx,
		`UPDATE sources SET status = $2, next_fetch_at = now() - interval '1 minute'
		  WHERE id = $1`, disabledSource, types.SourceStatusDisabled); err != nil {
		t.Fatalf("prepare disabled source: %v", err)
	}
	if _, err := st.pool.Exec(ctx,
		`UPDATE sources SET status = $2, next_fetch_at = now() + interval '1 hour'
		  WHERE id = $1`, futureSource, types.SourceStatusActive); err != nil {
		t.Fatalf("prepare future source: %v", err)
	}
	if _, err := st.pool.Exec(ctx,
		`UPDATE sources SET status = $2, next_fetch_at = now() - interval '1 minute'
		  WHERE id = $1`, outsideSource, types.SourceStatusActive); err != nil {
		t.Fatalf("prepare outside source: %v", err)
	}

	due, err := st.ListDueSourcesByIDs(ctx, []int64{sourceB, futureSource, sourceA, disabledSource})
	if err != nil {
		t.Fatalf("ListDueSourcesByIDs(): %v", err)
	}
	if len(due) != 2 {
	...
	displayID, ok, err := st.SourceForContentFromIDs(ctx, displayItem, []int64{displaySource})
	if err != nil {
		t.Fatalf("SourceForContentFromIDs(): %v", err)
	}
	if !ok || displayID != displaySource {
		t.Fatalf("display source = (%d,%v), want frozen source (%d,true)", displayID, ok, displaySource)
	}

	deterministicID, ok, err := st.SourceForContentFromIDs(ctx, displayItem, []int64{displaySource, sourceA})
	if err != nil {
		t.Fatalf("SourceForContentFromIDs() deterministic: %v", err)
	}
	...
	missingID, ok, err := st.SourceForContentFromIDs(ctx, displayItem, []int64{sourceB})
	if err != nil {
		t.Fatalf("SourceForContentFromIDs() empty intersection: %v", err)
	}
	if ok || missingID != 0 {
