...
//   - 无内容可推：直接 empty（008 起，见下）——**这是正常终态，不是失败**。
//
// "pushing" 已于 008 删除：它从 001 起就是死枚举，全仓零赋值、无任何 SQL 写过它，
...
const (
	...
	// BatchStatusEmpty 无内容可推的正常终态（008 / M5 契约 §16 修订记录「空批次缺口」）。
...
// 本身反而没法一句话查（得 IN 五个值）。
//
// 空串 = 没有提前退出（跑到了 Push）。008 之前的历史行全部如此，故列默认 ''
// 恰好就是它们的真实语义，无需回填。
//
