...
func TestValidateMaterializedRoundTripsEveryAvailableCapability(t *testing.T) {
	...
	for _, spec := range tests {
		...
		t.Run(spec.Platform+"/"+spec.Capability, func(t *testing.T) {
			source, message := Build(spec)
			if message != "" || source == nil {
				t.Fatalf("Build: source=%+v message=%q", source, message)
			}
...
func TestValidateMaterializedRejectsIdentityConfigDrift(t *testing.T) {
	...
	tests := []struct {
		name   string
		mutate func(*types.Source)
	}{
		{
			name: "runtime query differs",
			mutate: func(source *types.Source) {
...
func TestValidateMaterializedRejectsReservedFeedMarkerWithoutConfig(t *testing.T) {
	t.Parallel()
	categorized, message := Build(Spec{
	...
	if message != "" || categorized == nil {
		t.Fatalf("fixture Build failed: %q", message)
	}
	spoofed := *categorized
	...
	if source, message := Build(Spec{
		Platform: "web", Capability: "feed", Title: "spoofed feed",
		Params: map[string]string{"url": categorized.URL},
	}); message == "" || source != nil {
		t.Fatalf("Build accepted reserved category URL without config: source=%+v message=%q",
			source, message)
	}

	queryMarker, message := Build(Spec{
	...
	if message != "" || queryMarker == nil {
		t.Fatalf("query marker fixture Build failed: %q", message)
	}
